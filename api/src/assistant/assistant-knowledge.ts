...
export const ASSISTANT_ACTIONS: Record<string, AssistantAction> = {
  ...
  ticketTransfer: {
    id: "ticketTransfer",
    label: "Transfer a ticket",
    href: "/ticket-transfer",
  },
  trips: { id: "trips", label: "Plan a trip", href: "/trips" },
  newTrip: { id: "newTrip", label: "Create a trip", href: "/trips/new" },
  tripCommunity: {
    id: "tripCommunity",
    label: "Open trip community",
    href: "/trips/community",
  },
  help: { id: "help", label: "Open Help Center", href: "/help" },
  faq: { id: "faq", label: "Open FAQ", href: "/faq" },
  blog: { id: "blog", label: "Open Blog & Updates", href: "/blog" },
  notifications: {
    id: "notifications",
...
export const ASSISTANT_KNOWLEDGE: AssistantKnowledgeEntry[] = [
  ...
  },
  {
    ...
    answer:
      "Yes, hourly rental is available when a vehicle listing provides an hourly rate. On that listing, choose By hour, select a date, enter a start time and end time on the same day, and review the estimated total. The listing may also specify minimum rental hours and an additional hourly driver fee. Then send a request to the owner for approval. It is a request rather than an instant booking, and no payment is taken at that time.",
    actionIds: ["carRentals", "bookings"],
  ...
  {
    id: "ticket-transfer",
    title: "Transfer an event ticket safely",
    keywords: [
      "transfer ticket",
      "send ticket",
      "give ticket",
      "friend use my ticket",
      "someone else use my ticket",
      "ticket recipient",
      "ticket transfer",
      "claim transferred ticket",
    ],
    answer:
      "When ticket transfer is available for your event, open the ticket or transfer page, choose the pass, and use the transfer option to create or send the transfer link or token. The recipient must open that link or enter the token where LIBERIA360 asks, sign in if required, and claim the pass. A transferred pass should be used only by the accepted recipient and may no longer remain available to the sender. Never share a QR payload, password, verification code, or payment details in chat or with an untrusted person. The Assistant can explain the steps but cannot transfer or claim a ticket for you.",
    actionIds: ["myTickets", "ticketTransfer", "login"],
    followUps: [
      "Where can I see my QR ticket?",
      "How do organizers scan tickets?",
    ],
  },
  {
    id: "business-menus-orders",
    title: "Business menus and food orders",
    keywords: [
      "business menu",
      "add food",
      "add menu item",
    ...
    answer:
      "When menu tools are enabled for an approved business, an owner can open the account area to add and manage menu items, including the item name, description, price, category, availability, and image when those fields are provided. Customers can open the business profile to view its menu. Food ordering is available only for businesses that have ordering enabled; it is not supported by every business. When ordering is available, customers follow the menu’s order flow and owners manage incoming orders in the account area. Do not assume an order was placed or accepted unless the product shows a confirmation.",
    actionIds: ["myPlaces", "search", "account"],
    followUps: [
      "How do I add my business?",
      "Can every business accept food orders?",
    ],
  },
  {
    id: "trip-collaboration",
    title: "Create and collaborate on trips",
    keywords: [
      "invite someone to my trip",
    ...
    answer:
      "Create a trip, add places or stops to the itinerary, and use the trip’s sharing or invitation controls when available to invite people. Invitees can accept or decline from the invitation, while other users may request to join when the trip allows requests. Public trips can be discoverable, invited trips are limited to people who receive access, and private trips are restricted to the owner and approved participants. Collaborators can help plan stops and use trip community or trip chat only when those tools are available for that trip. The Assistant can guide you but cannot invite people or change trip access for you.",
    actionIds: ["trips", "newTrip", "tripCommunity", "notifications"],
    followUps: [
      "Where are my trip invitations?",
      "How do I add stops to a trip?",
    ],
  },
  {
    id: "event-discovery-actions",
    title: "Filter, share, and show interest in events",
    keywords: [
      "events this weekend",
      "events today",
      "events this month",
    ...
    answer:
      "Open Events to filter listings by category, county, and time period such as Today, This weekend, or This month. Free events do not require a ticket purchase through the paid-ticket flow; paid events may offer ticket types and payment instructions. Use Interested when that action is available, and use Share to send the event link. An authorized signed-in user can use Add an event to submit an event for review. The Assistant cannot confirm event availability or publish an event for you.",
    actionIds: ["events", "addEvent", "login"],
    followUps: [
      "How do I buy a ticket for a paid event?",
      "Where can I find events this weekend?",
    ],
  },
  {
    id: "near-me-map",
    title: "Near Me and Explore Map",
    keywords: [
      "near me",
      "places near me",
      "nearby places",
    ...
    answer:
      "Near Me uses your current location to help find places close to you, while Explore Map lets you browse places visually on a map. Your browser or device may ask for location permission. If you do not grant permission or location is unavailable, use Search, counties, or the map’s manual browsing controls instead; the Assistant cannot see your precise location from chat.",
    actionIds: ["nearMe", "map", "search"],
    followUps: ["How do I search by county?", "How do I save a place?"],
  },
  {
    id: "help-faq-blog-support",
    title: "Help Center, FAQ, Blog, and Customer Support",
    keywords: [
      "help center",
      "help guides",
      "faq",
    ...
    answer:
      "Help Center guides explain how to use specific parts of LIBERIA360, FAQ provides short answers to common questions, and Blog & Updates contains official announcements or articles when published. Customer Support is the official place to report a problem: open Account → Customer Support, choose the closest category, and include the relevant details. The Assistant should not claim that a Help article or blog post exists if the relevant page is empty.",
    actionIds: ["help", "faq", "support"],
    followUps: [
  ...
  },
  {
    ...
    title: "Business-owner account tools",
    keywords: [
    ...
    answer:
      "Approved business owners may manage their business profile, content, bookings, menu items, orders, and analytics from the account area when those tools are enabled for their account. Some submissions or changes require review before they become public. The available tools depend on the signed-in account and the features enabled for that business, so the Assistant should not promise a tool that is not shown in the account.",
    actionIds: ["account", "myPlaces", "bookings"],
    followUps: [
      "How do I add my business?",
  ...
  {
    id: "event-ticket-analytics",
    title: "Event and ticket analytics",
    keywords: [
      "ticket statistics",
      "ticket analytics",
      "event metrics",
      "event insights",
      "orders attendance scans",
      "ticket performance",
      "see my ticket statistics",
    ],
    answer:
      "Authorized event organizers may see event or ticket metrics such as orders, attendance, scans, and performance when those metrics are returned by the product. Open My Events and the event’s management or Insights area when available. Metrics depend on the event and the organizer’s permissions. The Assistant must not expose private attendee information or claim that a metric exists when it is not shown.",
    actionIds: ["myEvents", "events"],
    followUps: [
      "How do organizers scan tickets?",
      "How do I review ticket orders?",
    ],
  },
  {
    id: "notifications-invitations",
    title: "Notifications and invitations",
    keywords: [
    ...
    answer:
      "Open Notifications to review account alerts, booking updates, support updates, trip invitations, and other notifications that your signed-in account is allowed to see. Notification visibility depends on the account and available product events. If a trip invitation is not visible, confirm that you are signed in to the intended account and check the trip’s sharing or invitation page.",
    actionIds: ["notifications", "trips", "account"],
    followUps: [
      "How do I accept a trip invitation?",
      "Where are my booking messages?",
    ],
  },
  {
    ...
    answer:
      "When Creator Stories are available in the current LIBERIA360 environment, approved creators can publish photo or video Stories that are available for up to 24 hours. Stories may be public or limited to followers, and viewing, reporting, and publishing can require sign-in and the appropriate creator approval. Open Creators to view available Stories or the creator area to publish. If the Stories controls or API are unavailable, the Assistant should say that availability is not confirmed instead of claiming that Stories are active in production.",
    actionIds: ["creators", "creatorProfile", "login"],
    followUps: [
      "How do I become an approved creator?",
      "How do I report a Story?",
    ],
  },
  {
    id: "car-rental-filters-booking",
    title: "Car-rental filters and booking options",
    keywords: [
      "car rental filters",
      "vehicle category",
      "transmission filter",
      ...
      "county car rental",
      "driver availability",
      "hourly rental with driver",
      "pickup details",
    ],
    answer:
      "In Car Rentals, filter vehicles by category, transmission, county, minimum seats, maximum price per day, and driver availability when those filters are shown. Listings may support daily or hourly rental requests, with an optional driver fee and pickup details. A request is sent to the owner for approval; it is not necessarily an instant booking or payment. Review the listing’s available dates, rates, driver option, pickup information, and any deposit before sending a request.",
    actionIds: ["carRentals", "bookings"],
    followUps: [
  ...
  {
    id: "onboarding-saved-places",
    title: "Onboarding and saved places",
    keywords: [
      "onboarding tour",
      "getting started",
      "save a place",
      ...
      "where are saved items",
    ],
    answer:
      "The onboarding tour introduces the main LIBERIA360 areas and can help you get started. Use the save or bookmark control on a place or supported creator post to save it, then open Saved from your account to view saved items. Sign in may be required. Offline access or synchronization should not be assumed unless the product explicitly confirms it; saved items are normally retrieved from the account when the app is connected.",
    actionIds: ["saved", "account", "login"],
    followUps: [
      "How do I save a creator post?",
...
export const ASSISTANT_QUICK_PROMPTS = [
  ...
  "How do I transfer an event ticket?",
  "Can customers order from my menu?",
  "How do I invite someone to my trip?",
  "How do I find events this weekend?",
  "Where can I find places near me?",
  "What is the difference between Help and Customer Support?",
  "Where are my trip invitations?",
  "How do I see my ticket statistics?",
  "How do Creator Stories work?",
  "Can I rent a car by the hour with a driver?",
