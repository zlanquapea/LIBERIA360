...
describe("AnalyticsService", () => {
  ...
  let eventsRepo: { exists: jest.Mock; findOne: jest.Mock };
  ...
  describe("record", () => {
    ...
    it("404s an unknown event", async () => {
      eventsRepo.exists.mockResolvedValue(false);
      await expect(
        service.record({
          eventId: EVENT_ID,
          eventType: AnalyticsEventType.VIEW,
    ...
    it("records a view against an existing event", async () => {
      await service.record({
        eventId: EVENT_ID,
        eventType: AnalyticsEventType.VIEW,
      });
  ...
  describe("getEventAnalytics", () => {
    it("404s an unknown event", async () => {
      await expect(
        service.getEventAnalytics(OWNER_ID, EVENT_ID),
      ).rejects.toBeInstanceOf(NotFoundException);
    });

    it("403s a user who isn't the event's organizer", async () => {
      eventsRepo.findOne.mockResolvedValue({
        id: EVENT_ID,
        createdByUserId: OWNER_ID,
      });
      await expect(
        service.getEventAnalytics(STRANGER_ID, EVENT_ID),
      ).rejects.toBeInstanceOf(ForbiddenException);
    });

    it("returns zeroed totals for an event with no views yet", async () => {
      eventsRepo.findOne.mockResolvedValue({
        id: EVENT_ID,
        createdByUserId: OWNER_ID,
      });
      ...
      const result = await service.getEventAnalytics(OWNER_ID, EVENT_ID);
