...
describe("SupportService", () => {
  ...
  describe("assignment", () => {
    ...
    it("allows an administrator to return a ticket to the unassigned queue", async () => {
  ...
  it("does not allow submitted feedback to be overwritten", async () => {
    const { service } = setup({ rating: 5, ratingComment: "Already sent" });
    await expect(
      service.rate(customer, "ticket-1", {
        rating: 1,
        comment: "Replacement feedback",
      }),
    ).rejects.toBeInstanceOf(BadRequestException);
  });
});
