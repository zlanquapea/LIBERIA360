...
export class SupportService {
  ...
  async assignToSelf(agent: User, id: string) {
    const ticket = await this.get(id);
    if (ticket.assignedAgentUserId === agent.id) return ticket;
    if (ticket.assignedAgentUserId && ticket.assignedAgentUserId !== agent.id) {
      throw new BadRequestException(
        `This ticket is already assigned to ${ticket.assignedAgent?.name ?? "another support agent"}`,
      );
    }
    // Claim atomically. Two agents can open the same unassigned ticket, so
    // checking and then saving would allow the slower request to overwrite
    // the faster one. The null predicate makes the database arbitrate.
    const claimed = await this.tickets.update(
      { id, assignedAgentUserId: IsNull() },
      { assignedAgentUserId: agent.id },
    );
    if (!claimed.affected) {
      const current = await this.get(id);
      throw new BadRequestException(
        `This ticket is already assigned to ${current.assignedAgent?.name ?? "another support agent"}`,
      );
    }
    return this.get(id);
  }

  async update(_agent: User, id: string, dto: UpdateSupportTicketDto) {
    const ticket = await this.get(id);
  ...
  async rate(user: User, id: string, dto: RateSupportTicketDto) {
    ...
    ticket.ratingComment = dto.comment.trim();
    return this.tickets.save(ticket);
  }
}
