...
export class GuideBooking {
  ...
  @Column({ type: "enum", enum: GuideBookingStatus, default: GuideBookingStatus.REQUESTED })
  status: GuideBookingStatus;

  @Column({ name: "price_usd_snapshot", type: "numeric", precision: 10, scale: 2, transformer: decimalTransformer })
  priceUsdSnapshot: number;

  @Column({ name: "price_lrd_snapshot", type: "numeric", precision: 12, scale: 2, nullable: true, transformer: decimalTransformer })
  priceLrdSnapshot: number | null;

  @Column({ name: "payment_status", type: "enum", enum: GuidePaymentStatus, default: GuidePaymentStatus.UNPAID })
