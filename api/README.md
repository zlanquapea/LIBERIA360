...
- IP addresses recorded in the audit trail and login activity log (`src/common/request-info.ts`) come from Express's `req.ip`. Set `TRUST_PROXY_HOPS` to the exact number of trusted reverse proxies in front of the API so forwarded client addresses are interpreted safely; the default of `0` deliberately ignores forwarded headers for direct deployments.
