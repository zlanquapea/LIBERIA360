...
describe("Two-factor authentication (e2e)", () => {
  ...
  it("walks the full setup → enable → login → verify → recovery → disable flow", async () => {
    ...
    // Register — gets a real accessToken, 2FA not enabled yet.
    ...
    // Login now returns a pendingToken instead of a real accessToken.
    ...
    const loginAfterDisable = await request(app.getHttpServer())
      ...
      .send({ email, password })
      .expect(200);
    expect(loginAfterDisable.body.accessToken).toBeDefined();
