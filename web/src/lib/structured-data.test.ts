...
import type { Business, Category, County, Creator, Event, Place } from './types';
...
const CATEGORY: Category = { id: 'c1', name: 'Beaches', slug: 'beaches', description: 'Beach spots', icon: 'SunIcon' };
...
describe('placeJsonLd', () => {
  it('maps place type to the matching schema.org type', () => {
    ...
    expect(placeJsonLd({ ...PLACE, type: 'restaurant' })['@type']).toBe('Restaurant');
  });

  it('omits aggregateRating for a place with no reviews', () => {
    expect(placeJsonLd({ ...PLACE, reviewCount: 0 })).not.toHaveProperty('aggregateRating');
  });
});
...
describe('categoryJsonLd', () => {
  it('is a CollectionPage whose ItemList mirrors the given places', () => {
    const data = categoryJsonLd(CATEGORY, [PLACE]) as {
      '@type': string;
      mainEntity: { numberOfItems: number; itemListElement: Array<{ position: number; name: string }> };
    };
    expect(data['@type']).toBe('CollectionPage');
    expect(data.mainEntity.numberOfItems).toBe(1);
    expect(data.mainEntity.itemListElement[0]).toMatchObject({ position: 1, name: 'CeeCee Beach' });
  });
});
...
describe('businessJsonLd', () => {
  ...
  it('collects the website and social links into sameAs', () => {
    ...
    expect(data.sameAs).toEqual(['https://example.com', 'https://instagram.com/ceecee']);
  });
});
...
describe('creatorJsonLd', () => {
  ...
  it('wraps a Person in a ProfilePage, with social links collected into sameAs', () => {
    const data = creatorJsonLd(CREATOR) as {
      '@type': string;
      mainEntity: { '@type': string; name: string; sameAs: string[]; aggregateRating: { ratingValue: number } };
    };
  ...
  it('omits aggregateRating for a creator with no reviews', () => {
    const data = creatorJsonLd({ ...CREATOR, reviewCount: 0 }) as { mainEntity: object };
