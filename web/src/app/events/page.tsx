    ...
    <main className="mx-auto flex max-w-3xl flex-col gap-4 px-4 py-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Concerts, festivals, sports, and more — across Liberia.</p>
        </div>
        <Link
          href="/events/new"
          ...
          {result.data.map((event) => (
            <li key={event.id}>
              <Link
                ...
                className="flex items-start justify-between gap-3 rounded-xl border border-slate-200 p-3 hover:border-brand-500"
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-slate-900">{event.name}</p>
                  <p className="text-sm text-slate-600">{formatEventDateRange(event.startDate, event.endDate)}</p>
