  ...
  return (
    <main className="page-shell max-w-5xl">
      <PageHeader
        ...
        action={
          <Link href="/guides/apply" className="button-secondary">
            Become a guide
          </Link>
        }
      />
      <form className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="min-h-11 flex-1 rounded-2xl border border-slate-300 bg-transparent px-4 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-slate-700"
          />
        ...
        >
          <Link
            href={href()}
            className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold ${!category ? "bg-brand-700 text-white" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}
          >
            All
          </Link>
          {(["city", "culture", "nature", "food"] as const).map((item) => (
            <Link
              key={item}
              href={href(item)}
              className={`whitespace-nowrap rounded-full px-4 py-2 text-sm font-semibold ${category === item ? "bg-brand-700 text-white" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}
            >
              {label(item)}
            </Link>
          ))}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <input
            ...
            className="min-h-11 rounded-2xl border border-slate-300 bg-transparent px-4 text-sm dark:border-slate-700"
          />
          <input
            name="language"
      ...
      {guides.length === 0 ? (
        <p className="empty-state">
          No guides in this search yet. Try another county, language, or
          category.
        </p>
      ) : (
        <section
          className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"
          aria-label="Verified trip guides"
        >
          {guides.map((guide) => (
            <article
              key={guide.id}
            ...
            >
              ...
              <span className="mt-4 inline-flex w-fit rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-200">
                ✓ Verified
              </span>
              ...
              <Link
                ...
                className="button-secondary mt-5 min-h-11 w-full"
              >
                View Guide
              </Link>
            </article>
