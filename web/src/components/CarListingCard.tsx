...
export function CarListingCard({ listing, index }: { listing: CarListing; index?: number }) {
  ...
  return (
    // A plain wrapping div, not itself a Link — AddToTripButton below is a
    // sibling of the card's own Link, not nested inside it (a button
    // inside an anchor is invalid HTML and fights the anchor for clicks),
    // positioned over it via `relative`/`absolute` so browsing the fleet
    // and adding one to a trip don't require opening the detail page
    // first — the same "explore, then add" flow AddTripStop's own
    // "view details" link offers from the other direction.
