    ...
    <section className="ticket-purchase-shell" aria-labelledby="ticket-purchase-title">
      ...
      {ticketTypes.length > 0 && <div className="grid gap-3 sm:grid-cols-2">
        {ticketTypes.map((ticket) => {
          ...
          return <article key={ticket.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div className="flex items-start justify-between gap-2"><div><h3 className="font-bold text-slate-900 dark:text-white">{ticket.name}</h3><p className="mt-1 text-2xl font-black text-brand-700">{event.ticketCurrency} {Number(ticket.price).toFixed(2)}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase ${almostGone ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}`}>{almostGone ? 'Almost sold out' : 'Available'}</span></div>
            {ticket.description && <p className="mt-2 text-sm text-slate-500">{ticket.description}</p>}
            <div className="mt-4 flex items-center justify-between"><span className="text-xs font-semibold text-slate-500">Quantity</span><div className="flex items-center overflow-hidden rounded-full border border-slate-300"><button type="button" aria-label={`Remove one ${ticket.name}`} onClick={() => setQuantities((all) => ({ ...all, [ticket.id]: Math.max(0, count - 1) }))} className="h-10 w-10 text-lg">−</button><strong className="w-8 text-center">{count}</strong><button type="button" aria-label={`Add one ${ticket.name}`} onClick={() => setQuantities((all) => ({ ...all, [ticket.id]: Math.min(ticket.quantity, count + 1) }))} className="h-10 w-10 text-lg">+</button></div></div>
          </article>;
        })}
      </div>}
      ...
      {!user ? (
        ...
        <div className="ticket-purchase-form">
          ...
          <button type="button" disabled={submitting || !paymentReference.trim() || (ticketTypes.length > 0 && selectedQuantity === 0)} onClick={submit} className="ticket-submit-button">
