...
function pinIcon(color: string, icon: string | null) {
  return L.divIcon({
    ...
    html: `<div style="background:${color}" class="flex h-8 w-8 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 border-white shadow-md">${iconSvgMarkup(icon, 'h-4 w-4 text-white')}</div>`,
    iconSize: [32, 32],
    iconAnchor: [16, 16],
  });
}
