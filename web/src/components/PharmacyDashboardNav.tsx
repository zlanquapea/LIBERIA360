...
import { PHARMACY_DASHBOARD_NAV, pharmacyDashboardHref } from '@/lib/pharmacy-dashboard-nav';

// Drives both the desktop sidebar and the mobile horizontal scroll-tab
// strip from the same item list — identical structure to
// BusinessDashboardNav, just pointed at the pharmacy dashboard's own
// (unconditional) tab list.
export function PharmacyDashboardNav({ pharmacyId }: { pharmacyId: string }) {
  const pathname = usePathname();
  const items = PHARMACY_DASHBOARD_NAV;

  return (
    <>
      {/* Mobile: horizontal scroll-snap tab strip */}
      <nav
        aria-label="Pharmacy dashboard sections"
        className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 lg:hidden"
      >
        {items.map((item) => {
          const href = pharmacyDashboardHref(pharmacyId, item.segment);
