...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-white/10 bg-lagoon-900/95 text-white shadow-[0_8px_24px_rgba(0,67,84,0.16)] backdrop-blur-xl supports-[backdrop-filter]:bg-lagoon-900/90">
      <div className="mx-auto flex min-h-[4.5rem] max-w-[90rem] items-center justify-between gap-3 px-3 py-1.5 sm:px-6 lg:px-10">
        ...
        >
          {SITE_NAVIGATION.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-3 py-2 text-sm font-semibold text-lagoon-100 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400"
            >
              {t(item.labelKey)}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <Link
            href="/search"
            className="flex min-h-10 items-center gap-1.5 rounded-full border border-white/25 bg-white/10 px-3 py-1.5 text-sm font-semibold text-white/90 transition-all hover:border-white hover:bg-white hover:text-lagoon-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400"
          >
