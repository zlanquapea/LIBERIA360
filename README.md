...
- **`web`**: builds the API, runs migrations, seeds Stage 1 sample data, and starts it for real (`npm run start:prod`, polled on `/health` before continuing) — `next build` statically prerenders a handful of pages (`/`, `/counties`, `/explore`, ...) that fetch from the API *at build time*, so without a live, seeded API those builds fail outright rather than just missing data. Then `next lint`, `tsc --noEmit`, and `next build`.
